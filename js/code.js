...
const urlBase = (typeof window !== 'undefined' && window.location && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.origin.includes('disc.quest')))
  ? '/api/index.php'
...
function addContact() {
    ...
    if (!newFirstName || !newLastName || !newPhoneNum || !newEmailAdd) {
        resultEl.className = "text-warning small fw-semibold";
        resultEl.innerHTML = "<i class='bi bi-exclamation-triangle-fill me-1'></i> Please enter all feilds";
        return;
    }


    let jsonPayLoad = JSON.stringify({ firstname: newFirstName, lastname: newLastName, phone: newPhoneNum, email: newEmailAdd });
...
function addAccount() {
    ...
    if (!newLogin || !newPassword || !newFirstName || !newLastName) {
        resultEl.className = "text-warning small fw-semibold";
        resultEl.innerHTML = "<i class='bi bi-exclamation-triangle-fill me-1'></i> Please enter all required information";
        return;
    }

    let jsonPayload = JSON.stringify({ firstName: newFirstName, lastName: newLastName, login: newLogin, password: newPassword });
    ...
    try {
        xhr.onreadystatechange = function () {
            if (this.readyState === 4) {
                if (this.status === 201 || this.status === 200) {
                    resultEl.className = "text-success-wcag small fw-semibold";
                    resultEl.innerHTML = "<i class='bi bi-check-circle-fill me-1'></i> New user successfully created";
                    newLoginInput.value = "";
...
function addContact() {
    ...
    let jsonPayload = JSON.stringify({ firstName: newFirstName, lastName: newLastName, email: newEmail, phoneNumber: newPhone });
    let url = urlBase;
    ...
    try {
        xhr.onreadystatechange = function () {
            if (this.readyState === 4) {
                if (this.status === 201 || this.status === 200) {
                    resultEl.className = "text-success-wcag small fw-semibold";
                    resultEl.innerHTML = "<i class='bi bi-check-circle-fill me-1'></i> New contact successfully created";
                    newEmailInput.value = "";
                    newPhoneInput.value = "";
                    newFirstNameInput.value = "";
                    newLastNameInput.value = "";
...
if (!newContactID || !newEmail || !newPhone || !newFirstName || !newLastName) {
    ...
    try {
        xhr.onreadystatechange = function () {
            if (this.readyState === 4) {
                if (this.status === 201 || this.status === 200) {
                    resultEl.className = "text-success-wcag small fw-semibold";
                    resultEl.innerHTML = "<i class='bi bi-check-circle-fill me-1'></i> Contact successfully updated";
                    newEmailInput.value = "";
                    newPhoneInput.value = "";
                    newFirstNameInput.value = "";
                    newLastNameInput.value = "";
...
function deleteContact() {
    ...
    let deleteContactID = deleteContactIDInput ? deleteContactIDInput.value.trim() : "";
    ...
    if (!deleteContactID) {
        ...
        resultEl.className = "text-warning small fw-semibold";
        resultEl.innerHTML = "<i class='bi bi-exclamation-triangle-fill me-1'></i> Please enter an ID to remove by";
    ...
    try {
        xhr.onreadystatechange = function () {
            if (this.readyState === 4) {
                if (this.status === 201 || this.status === 200) {
                    resultEl.className = "text-success-wcag small fw-semibold";
                    resultEl.innerHTML = "<i class='bi bi-check-circle-fill me-1'></i> Contact successfully deleted";
                    deleteContactID = "";
